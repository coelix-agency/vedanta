...
if( !$is_child_parent || count( $all_terms_name ) > 0 ) {
    ...
    if( is_array($all_terms_slug) ) {
        ...
        foreach($all_terms_slug as &$all_term_slug) {
	        ...
	        if(strripos($all_term_slug, wpm_get_language())) {
		        if (preg_match('!\[:' . wpm_get_language() . ']([^\)]+)\[!', $all_term_slug, $match)) {
			        $all_term_slug = $match[1];
			        $all_term_slug = strtok($all_term_slug, '[');
		        }
	        }
