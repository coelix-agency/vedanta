...
if ( ! empty($terms) ):
    ...
    if( ! empty($hide_child_attributes) && in_array(berocket_isset($term, 'parent'), $added_categories) ) {
        ...
        <div class="filters__list-group--checkbox">
        ...
        <span>
            ...
            <label data-for='checkbox_<?php echo str_replace ( '*' , '-' , berocket_isset($term, 'term_id')), str_replace ( '*' , '-' , $term_taxonomy_echo) ?>' class="berocket_label_widgets<?php if( br_is_term_selected( $term, true, $is_child_parent_or, $child_parent_depth ) != '') echo ' berocket_checked'; ?>">
                ...
                <?php
                $text = $term->name;
                if(strripos($term->name, wpm_get_language())) {
                    if (preg_match('!\[:' . wpm_get_language() . ']([^\)]+)\[!', $text, $match)) {
                        $text = $match[1];
                        $text = strtok($text, '[');
                    }
                }
