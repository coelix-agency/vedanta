...
if ( is_array(berocket_isset($terms)) ) {
    foreach ( $terms as $term ) { 
        ...
        if ( $is_child_parent && $is_first ) {
            ?><li class="berocket_child_parent_sample">
        ...
        ?><li class="radio-list berocket-radio berocket_term_parent_<?php echo berocket_isset($term, 'parent'); ?> berocket_term_depth_<?php
            echo $parent_count;
            ...
            <input class="<?php echo br_get_value_from_array($uo, array('class', 'checkbox_radio')); ?> radio_<?php echo berocket_isset($term, 'term_id') ?>_<?php echo $term_taxonomy_echo;
                ...
                echo ' '.$data_jquery;
                ?>/><label data-for='radio_<?php echo berocket_isset($term, 'term_id') ?>_<?php echo $term_taxonomy_echo ?>' class="berocket_label_widgets<?php
                    if( br_is_term_selected( $term, true, $is_child_parent_or, $child_parent_depth ) != '') echo ' berocket_checked'; 
                       ?>">

                <?php

                if( $term->taxonomy == '_stock_status' ) :
                    if($term->term_id == 2) :
	                    echo __( 'В пути', 'vedanta' ) . ' (' . $term->count . ')';
                    else:
	                ...
	                echo apply_filters( 'berocket_check_radio_color_filter_term_text', ( ( ! empty($icon_before_value) ? ( ( substr( $icon_before_value, 0, 3) == 'fa-' ) ? '' : '' ) : '' ) .
	                                                                                     apply_filters('berocket_radio_filter_term_name', berocket_isset($term, 'name'), $term) .
	                                                                                     ( ! empty($icon_after_value) ? ( ( substr( $icon_after_value, 0, 3) == 'fa-' ) ? '' : '' ) : '' ) ), $term, $operator, TRUE ) . ' (' . $term->count . ')';
